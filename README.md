...
순수 개발 경력 자체는 길지 않습니다.  
하지만 약 10년간 **개발 → 기획 → 마케팅 → 창업 → 다시 개발**을 거치며  
...
> 고객 문제 정의부터 실행 가능한 솔루션까지 끝까지 책임지는 개발자를 지향합니다.
...
|---|---|---|
| 순수 코드 경력 | 5~10년+ | 짧음 (단, AI Agent 협업 사이클 운영 능력은 시니어 수준) |
| 비즈니스 이해 | 도메인 의존 | **9년 IT 기획·마케팅 주도** — 블록체인 거래소, 음악 스트리밍, 기술 마케팅 |
| 고객·사용자 관점 | 종종 약함 | **직접 창업**으로 브랜딩·물류·CS까지 부딪힘 |
| 요구사항 정의 | PM 의존 | 본인이 **PM** — 공공기관 프로젝트 포함 |
| AI 활용 깊이 | 도구 사용 수준 | **Skill Creator로 커스텀 스킬 제작** + Agent 사이클 설계 |
...
- 🎨 **신기능을 출시 직후 흡수** — Claude.ai의 디자인 도구가 공개된 직후 4개 프로젝트(AXBrief · MarkUI · Haru Ending · StageCaption)에 즉시 도입, 디자이너 없이도 1인 개발의 디자인 한계를 넘는 완성도와 톤·정보 위계를 확보
...
|---------------------|----------|
| 고객 문제 정의 | 9년간 기획·마케팅 / 창업 / 공공기관 PM 경험으로 모호한 요구를 시스템으로 변환 |
| 비즈니스 이해력 | 블록체인 거래소, 음악 스트리밍, 직접 창업까지 |
| AI 협업 사이클 설계 | Claude Code + Codex CLI 조합으로 1인 풀스택 사이클 + 팀 사이클 모두 운영 경험 |
| AI를 제품 안에 녹이기 | Claude API · OpenAI Realtime API로 실제 가치 창출 |
| 커스텀 AI 스킬 제작 | Claude Code **Skill Creator**로 프로젝트·상황에 맞는 스킬 직접 제작·운영 |
| 신기능 빠른 흡수 | Claude.ai 디자인 도구 출시 직후 4개 프로젝트에 도입 — 디자이너 없이 완성도 확보 |
| 풀스택 실행력 | Python · TypeScript · Next.js · Expo · Supabase · Railway |
| 팀 협업·PM 경험 | Notion · Linear · GitLab 위 AI Agent 사이클을 PM 역할로 운영 |

---

## 이전 직장 프로젝트 — 공공기관 AI Agent 활용 PM

> 국내 공공 연구기관 프로젝트에서 **PM 역할로 대규모 프론트엔드 마이그레이션을 AI Agent 중심으로 진행**한 경험입니다.  
> 단순히 AI를 보조로 쓴 것이 아니라, **개발 사이클 자체를 AI Agent 위에서 재설계**한 사례입니다.
...
- **협업 도구** : **Notion**(문서·요구사항·회의록) · **Linear**(태스크·스프린트) · **GitLab**(코드·MR·CI)

**🧭 의사결정 — Vue 3 검토 후 React 전환**
- 초기 방향은 Vue 3 마이그레이션이었으나, **정적 분석으로 의존성 부담**(특정 UI 라이브러리 70+ 파일 의존, Vue 3 미지원 패키지 다수)을 확인한 뒤
- **장기 운영성 · 채용 풀 · 생태계** 관점에서 **React 전환을 PM 의사결정으로 진행**
- 단순히 "AI한테 시켰다" 가 아니라, **기술 부채와 비즈니스 트레이드오프를 분석해 방향을 잡는 PM의 본질적 일**을 수행

**🤖 AI Agent 중심 개발 사이클 (핵심 차별점)**

**1. Claude Code + Superpowers 기반 마이그레이션**
- 대부분의 코드 변환 · 컴포넌트 재작성 · 테스트 작성을 **Claude Code Agent로 자동화**
- Superpowers 플러그인의 스킬 시스템 위에서 작업 일관성과 검증 사이클 유지

**2. 직접 스킬을 만들어 쓰는 운영자**
- 기존 스킬에만 의존하지 않고, **Claude Code의 Skill Creator로 프로젝트·상황 맞춤 스킬을 직접 제작**
- 대표 사례: **GitLab 자동 리뷰 스킬** — MR 단위 변경을 자동 분석하고 코멘트하는 스킬을 직접 설계·운영
- "AI를 쓰는 사람" 이 아니라 **"AI를 자기 워크플로우에 맞게 만드는 사람"** — AX 직군의 본질

**3. PM × AI Agent 협업 사이클 표준화**
- 팀원 작업 → AI Agent 자동 검증·리뷰 → PM 최종 승인 → 머지의 흐름을 표준화
- Notion(요구사항·문서) · Linear(태스크) · GitLab(코드) 위에 AI Agent 산출물을 연결해 **single source of truth** 유지

**🏛️ 공공기관 환경에서의 의미**
- 공공기관 환경은 **보안 · 운영 제약**이 까다로움에도 불구하고, **AI Agent를 중심에 둔 개발 사이클을 실제 적용한 흔치 않은 레퍼런스**
- 마이그레이션 작업의 **대부분을 진행**했고, AI 자동 리뷰 스킬·마이그레이션 패턴·문서를 **후임이 인계받을 수 있는 형태로 정리**한 뒤, 다음 도전(개인 프로덕트 운영)을 위해 이직

> *공공기관 + Vue → React 대규모 마이그레이션 + PM 역할 + AI Agent 사이클 자체 설계 — 이 조합은 국내에서 거의 보기 어려운 레퍼런스입니다.*

---

## 고객 납품 — Forward Deployed 스타일

> 개인 프로덕트이지만, 멀티테넌트 구조와 실제 고객사 납품 단계까지 끌고 간 사례입니다.
...
> 📅 6일 4시간 · 🧾 94 commits · ✨ Conventional 98.9% · 🤖 Claude Co-Auth 84% · 🎨 Claude.ai 디자인 (Editorial)
...
**🎨 Claude.ai 디자인 도구 (출시 직후 도입)**
- 출시 직후의 **Claude.ai 디자인 도구를 즉시 도입**해 Phase 3-A의 'Editorial' 톤 디자인 시스템 설계
- 1인 개발자임에도 전문 미디어 사이트 수준의 톤·타이포·정보 위계 확보
- 디자인 방향 확정 과정도 ROADMAP에 명문화 — `docs(roadmap): Phase 3-A 디자인 방향 Editorial 확정 기록`
...
> 📅 7일 · 🧾 10 commits · ✨ Conventional 100% · 🤖 Codex 중심 · 🎨 Claude.ai 디자인
...
**🎨 Claude.ai 디자인 도구 (출시 직후 도입)**
- 송출 화면(lower-third · side-panel) · 운영자 콘솔 · 모바일 QR 자막 진입 화면을 **Claude.ai 디자인 도구**로 설계
- 현장 AV/LED 운영을 전제로 한 디스플레이 가독성·정보 위계를 디자이너 없이 빠르게 정리
...
> 📅 23일 · 🧾 22 commits · ✨ Conventional 90.9% · 🤖 Codex 중심 · 🎨 Claude.ai 디자인 (Soft Bloom)
...
**🎨 Claude.ai 디자인 도구 (Soft Bloom · 출시 직후 도입)**
- 출시 직후의 **Claude.ai 디자인 도구**로 'Soft Bloom' 디자인 시스템 설계 → Expo 앱 핵심 화면에 정식 적용
- 모바일 앱 특유의 부드러운 톤·일러스트 분위기를 1인 개발에서 빠르게 확보
...
> 📅 30일 · 🧾 126 commits · ✨ Conventional 73.8% · 🤖 Codex CLI 중심 · 🎨 Claude.ai 디자인 (핸드오프)
...
**🎨 Claude.ai 디자인 도구 (핸드오프 · 출시 직후 도입)**
- 출시 직후의 **Claude.ai 디자인 도구**로 디자인 핸드오프 자료 생성 → `design-handoff/` 폴더에 시안·토큰 관리 → 코드로 구현
- 디자이너 없이도 SaaS 수준의 UI 일관성과 공통 컴포넌트 체계 확보
...
**AI 협업 도구 (개발 사이클)** : Claude Code (+ **Skill Creator로 커스텀 스킬 제작**), Codex CLI, Claude.ai (디자인 도구), Superpowers, Task Master, MCP

**팀 협업 도구** : Notion (문서·요구사항) · Linear (태스크·스프린트) · GitLab / GitHub
...
순수 개발 경력은 길지 않습니다.  
하지만 AI 시대의 AX · FDE 직군이 진짜 필요로 하는 것은  
**"문제를 정의하고, 운영을 책임지고, 새로운 도구를 자기 사이클에 흡수하는 사람"** 이라고 믿습니다.

매일 조금씩 더 나은 개발자가 되고 있습니다.
